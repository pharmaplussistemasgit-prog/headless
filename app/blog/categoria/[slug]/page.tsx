...
export async function generateMetadata({ params }: CategoryPageProps) {
    ...
    if (!category) return { title: 'Categoría no encontrada | PharmaPlus' };

    return {
        title: `${category.name} - Blog | PharmaPlus`,
        description: `Artículos y novedades sobre ${category.name}.`,
    };
}
    ...
    return (
        <div className="bg-gray-50 min-h-screen pb-20">
            ...
            <div className="bg-white border-b border-gray-200 py-16 text-center">
                <span className="text-[var(--color-pharma-blue)] font-bold tracking-wider text-sm uppercase mb-2 block">
                    Categoría
                </span>
                <h1 className="text-4xl font-bold text-gray-900 mb-4">{category.name}</h1>
                <p className="text-gray-500 max-w-2xl mx-auto px-4">
                    Explorando todos los artículos relacionados con {category.name}.
                </p>
            </div>

            <div className="w-full max-w-[95%] lg:max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 mt-12 flex flex-col lg:flex-row gap-12">
                {/* Main Content */}
                <div className="flex-1">
                    {posts.length === 0 ? (
                        <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-200">
                            <div className="text-6xl mb-4">📝</div>
                            <h3 className="text-xl font-bold text-gray-900 mb-2">No hay artículos aún</h3>
                            <p className="text-gray-500">
                                Estamos preparando contenido para esta categoría.
                            </p>
                            <Link href="/blog" className="inline-block mt-6 text-[var(--color-pharma-blue)] font-bold hover:underline">
                                ← Volver al Blog
                            </Link>
                        </div>
                    ) : (
